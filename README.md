...
Ecco automatically gives each correlated send a durable retry identity from the sender,
message kind, and `in_reply_to` envelope ID. It saves the exact signed envelope in
`$ECCO_HOME/outbox.sqlite3` before the network call. A retry of the same operation and
logical input reuses that envelope, so relay envelope-ID deduplication produces one message
when a prior response was ambiguous. A changed retry fails instead of creating a second
envelope. Callers do not create or pass an idempotency key. Saved reservations expire after
seven days, which is longer than the maximum Ecco Ops dispatcher thread lifetime.
...
Ecco uses it for automatic durable retry identity.
